...
class DetailBuilder:
    ...
    def build_chimney(self, ctx: Ctx) -> None:
        ...
        ctx.editor.placeBlock((cx, top_y + 1, cz), Block(mat_smoke, smoke_props))
    ...
    def build_interior(self, ctx: Ctx) -> None:
        ...
        if ctx.z < ct_z < ctx.z + ctx.d - 1:
            ctx.editor.placeBlock((ctx.x + 1, fy, ct_z), Block("minecraft:crafting_table"))
    ...
    def build_details(self, ctx: Ctx) -> None:
        ...
        # Lantern above door — use place_light for safe hanging state
        ...
        ctx.editor.placeBlock((door_x, base_y, outside_z), Block(flower_id, flower_props))
