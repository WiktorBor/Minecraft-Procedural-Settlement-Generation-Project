...
def clear_fortification_path(ctx, sx, sz, length, horizontal, tower_width, wall_top_y, heightmap, area):
    """Clears all blocks in the path of the wall to remove trees/obstructions."""
    padding = 2  # Extra space around the tower width to clear leaves
    
    for i in range(length):
        curr_x = sx + (i if horizontal else 0)
        curr_z = sz + (0 if horizontal else i)
        
        # Get ground height at this specific spot
        li = max(0, min(heightmap.shape[0] - 1, curr_x - area.x_from))
        lj = max(0, min(heightmap.shape[1] - 1, curr_z - area.z_from))
        gy = int(heightmap[li, lj])

        # Clear a volume from ground to above the wall
        # We clear a width of tower_width + padding
        for offset in range(-padding, tower_width + padding):
            for y in range(gy, wall_top_y + 10):
                if horizontal:
                    ctx.place_block((curr_x, y, sz + offset), Block("minecraft:air"))
                else:
                    ctx.place_block((sx + offset, y, curr_z), Block("minecraft:air"))