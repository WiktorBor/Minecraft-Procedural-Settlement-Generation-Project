...
class SquareCentre:
    ...
    def build(
        ...
        if radius < 3:
            self._build_simple_paving(editor, cx, cy, cz, plot.width, plot.depth)
            return
        ...
        self._fill_foundation(editor, cx, cz, effective_radius + 2, cy)
        ...
        for ix in range(cx - effective_radius, cx + effective_radius + 1):
            for iz in range(cz - effective_radius, cz + effective_radius + 1):
                if math.sqrt((ix - cx) ** 2 + (iz - cz) ** 2) <= effective_radius:
                    ...
                    editor.placeBlock((ix, cy, iz), Block(b_type))
        ...
        if fountain_style == "grand_spire":
            self._build_grand_spire(editor, cx, cy, cz, effective_radius)
        else:
            self._build_small_fountain(editor, cx, cy, cz, effective_radius)
    ...
    def _build_grand_spire(
        ...
        for dx in range(-radius, radius + 1):
            for dz in range(-radius, radius + 1):
                ...
                if r_inner_sq < dist_sq <= r_outer_sq:
                    editor.placeBlock((cx + dx, cy + 1, cz + dz), Block("minecraft:stone_bricks"))
                    ...
                    editor.placeBlock((cx + dx, cy + 1, cz + dz), Block("minecraft:water"))

        # --- Central mossy stone-brick column with tapering slab ledges ---
        # Slab ledges at these Y offsets shrink in radius as we go up,
        # creating the tapering water-spire silhouette seen in the screenshot.
        ...
        # --- Cap ---
        editor.placeBlock((cx, cy + 22, cz), Block("minecraft:water"))
        editor.placeBlock((cx, cy + 23, cz), Block("minecraft:oak_trapdoor", {"open": "false"}))
        editor.placeBlock((cx, cy + 24, cz), Block("minecraft:glowstone"))
        ...
        for r, h, block in tiers:
            ...
            for dx in range(-r, r + 1):
                for dz in range(-r, r + 1):
                    if dx ** 2 + dz ** 2 <= r * r:
                        editor.placeBlock((cx + dx, cy + h, cz + dz), Block(block))

        basin_r = max(1, int(tiers[-1][0] * 0.8))
        for dx in range(-basin_r, basin_r + 1):
            for dz in range(-basin_r, basin_r + 1):
                if dx ** 2 + dz ** 2 <= basin_r ** 2:
                    editor.placeBlock((cx + dx, cy + 3, cz + dz), Block("minecraft:water"))
                    editor.placeBlock((cx + dx, cy + 2, cz + dz), Block("minecraft:sea_lantern"))
    ...
    def _fill_foundation(
        ...
        """Fill ground beneath the fountain with stone to prevent water leaks."""
        for dx in range(-radius, radius + 1):
            for dz in range(-radius, radius + 1):
    ...
    def _build_simple_paving(
        ...
        for ix in range(cx - w // 2, cx + w // 2):
            for iz in range(cz - d // 2, cz + d // 2):
                editor.placeBlock((ix, cy, iz), Block("minecraft:stone_bricks"))