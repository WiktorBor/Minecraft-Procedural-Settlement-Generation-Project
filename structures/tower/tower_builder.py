...
class TowerBuilder:
    ...
    def _build(self, x: int, y: int, z: int) -> None:
        ...
        # --- Foundation ---
        geo.placeCuboid(self.editor,
            (x, y - 2, z), (x + w - 1, y - 1, z + w - 1),
            Block(found_block))

        # --- Hollow body ---
        geo.placeCuboidHollow(self.editor,
        ...
        # Each layer: place stair ring, step in 1 block each side
        roof_base = y + h
        peak      = w // 2
        for layer in range(peak):
            ...
            # East/west slopes (corners already placed above)
        ...
        # Proper battlements: 2-wide merlon, 1-wide gap (not alternating single blocks)
        parapet_y = roof_base
        geo.placeCuboidWireframe(self.editor,
            (x, parapet_y - 1, z), (x + w - 1, parapet_y - 1, z + w - 1),
            Block(wall_block))
        # Merlons: pairs of blocks at even start positions
        for i in range(w):
            for j in range(w):
        ...
        # --- Corner lanterns ---
        for lx, lz in [(x, z), (x+w-1, z), (x, z+w-1), (x+w-1, z+w-1)]:
        ...
        # --- Ladder on back wall ---
        geo.placeLine(self.editor,
            (door_x, y + 1, z + w - 1),
            (door_x, y + h - 1, z + w - 1),
            Block("minecraft:ladder", {"facing": "south"}))

        # --- Banner/flag on top of roof peak ---
        flag_y  = roof_base + peak + (1 if w % 2 == 1 else 0)
        flag_colors = ["purple", "blue", "red", "white"]
        import random
        color = random.choice(flag_colors)
        # Place a fence post as flagpole then a banner
        self.editor.placeBlock(
            (x + w // 2, flag_y, z + w // 2),
            Block(f"minecraft:{color}_banner",
                  {"facing": "south", "rotation": "0"}))
