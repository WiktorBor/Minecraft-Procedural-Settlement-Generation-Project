...
test_dock.py
------------
Standalone dock placement tester.

Run from the project root while your Minecraft server is running:

    python debug/test_dock.py

Builds a wooden fishing dock: oak-plank deck, dark-oak log pillars,
cobblestone bollards, oak fence railings, and lanterns.

The dock is two-part: a main deck (full-width, ~60 % of depth) and a
narrower finger pier extending toward the water.
...
- ROTATION      — dock pier direction: 0=south, 90=west, 180=north, 270=east
...
PLOT_WIDTH: int = 12
PLOT_DEPTH: int = 16   # deeper → longer finger pier

PALETTE_NAME: str = "medieval"

CLEAR_FIRST: bool = True

# Pier direction: 0=south, 90=west, 180=north, 270=east
ROTATION: int = 0
...
def main() -> None:
    ...
    plot = Plot(x=hx, y=hy, z=hz, width=PLOT_WIDTH, depth=PLOT_DEPTH, type="fishing")
    ...
    logger.info(
        "Building dock %dx%d at (%d,%d,%d) rotation=%d palette=%s ...",
        PLOT_WIDTH, PLOT_DEPTH, hx, hy, hz, ROTATION, PALETTE_NAME,
    )

    buf = Dock().build(editor, plot, palette, rotation=ROTATION)
    if not buf:
        logger.error("Dock returned an empty buffer.")
        sys.exit(1)
