...
test_farm.py
------------
Standalone farm placement tester.

Run from the project root while your Minecraft server is running:

    python debug/test_farm.py

Builds a medieval farm — timber-framed basin with farmland, water channel,
and crops — on the specified plot.
...
- SEED          — fixed int for reproducible crop layout, None for random
"""
...
def main() -> None:
    ...
    plot = Plot(x=hx, y=hy, z=hz, width=PLOT_WIDTH, depth=PLOT_DEPTH, type="farming")
    ...
    logger.info(
        "Building farm %dx%d at (%d,%d,%d) palette=%s ...",
        PLOT_WIDTH, PLOT_DEPTH, hx, hy, hz, PALETTE_NAME,
    )

    buf = Farm().build(plot, palette)
    if not buf:
        logger.error("Farm returned an empty buffer.")
        sys.exit(1)
