...
def main() -> None:
    ...
    logger.info(
        "Building blacksmith %dx%d at (%d,%d,%d) rotation=%d palette=%s ...",
        PLOT_WIDTH, PLOT_DEPTH, hx, hy, hz, ROTATION, PALETTE_NAME,
    )

    buf = Blacksmith().build(plot, palette, rotation=ROTATION)
    if not buf:
        logger.error(
            "Blacksmith returned an empty buffer — check PLOT_WIDTH >= 9 and PLOT_DEPTH >= 8."
        )
        sys.exit(1)
